class CrownWindow {
  ...
  public CrownWindow(PVector top_left, PVector bottom_right, float layer_thickness) {
    layers = new ArrayList<Layer>();
  ...
  public CrownWindowVertical(PVector top_left, PVector bottom_right, float layer_thickness) {
    ...
    for (int i = 0; i < num_layers; i++) {
      ...
      if (right_bound - left_bound >= 5) {
        Layer l = new VerticalLayer(left_bound, right_bound, x, layer_thickness);
        layers.add(l);
      }
    }
    // right half
    for (int i = 1; i <= num_layers; i++) {
      ...
      float left_bound = center_y - sqrt(sq(circle_radius) - sq(x - center_x));
      if (right_bound - left_bound >= 5) {
        Layer l = new VerticalLayer(left_bound, right_bound, x-layer_thickness, layer_thickness);
        layers.add(l);
      }
    }
  }
