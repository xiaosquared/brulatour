class Wall {
  ...
  public void addBrick(float min_brick_width, float max_brick_width) {
    ...
    float brick_height_multiplier = floor(random(1, min(4, bars.size() - current_bar_index)));
    float brick_height = bar_height * brick_height_multiplier;
    float brick_y = wall_height - ((current_bar_index + 1) * bar_height) - brick_height;
    
    
        
    Bar current_bar = bars.get(current_bar_index);
    float brick_position = current_bar.addBrick(min_brick_width, max_brick_width, 
                                                      brick_y, brick_height, bricks);
    

    if (brick_position >= 0  && brick_height_multiplier > 1) {
      for (int i = 0; i < brick_height_multiplier; i++) {
        Bar upper_bar = bars.get(current_bar_index + i);
        if (upper_bar.getInterval(brick_position) != null) {
...
class Bar {
  ...
  public Rectangle addBrick(float min_brick_width, float max_brick_width, 
    ...
    if (current_interval.getDistance() <= max_brick_width) {
      Rectangle brick = new Rectangle(current_interval.getLeft(), brick_y, current_interval.getDistance(), brick_height);
      bricks.add(brick);
      intervals.remove(current_interval);
      //return(intervals.isEmpty());
    }
    ...
    Rectangle brick = new Rectangle(position, brick_y, brick_width, brick_height);
    bricks.add(brick);
