class Roof extends Story implements Fillable {
  ...
  public Roof(float x, float y, float width, float height, float angle, boolean right, float layer_thickness, int hue) {
    super(x, y, width, height, layer_thickness, hue);
    
    this.angle = angle;
    this.right_incline = right;
    this.left_incline = !right;
    
    createInclineSides(main, x, y, angle, left_incline, right_incline);
  }
  
  // if triangle roof, just draw it in the middle 
  
  void addChimney(float where, float width, float height) {
    Layer top_layer = main.layers.get(main.layers.size()-1);
    int num_layers = floor(height / layer_thickness);
    
    if (top_layer.length < width) {
      float diff = width - top_layer.length;
      top_layer.lower_bound -= diff/2;
      top_layer.upper_bound += diff/2;
      top_layer.length = top_layer.upper_bound - top_layer.lower_bound;
      top_layer.slots.clear();
      top_layer.slots.add(new Slot(top_layer.lower_bound, top_layer.upper_bound));
      
      for (int i = 1; i < num_layers; i++) {
        Layer l = new Layer(top_layer.lower_bound, top_layer.upper_bound, top_layer.position - i * layer_thickness, top_layer.thickness);
        main.layers.add(l);
      }
    }
    else {
      float lower_bound = lerp(top_layer.lower_bound, top_layer.upper_bound - width, where);
      for (int i = 1; i <= num_layers; i++) {
        Layer l = new Layer(lower_bound, lower_bound + width, top_layer.position - i * layer_thickness, top_layer.thickness);
        main.layers.add(l);
      }
    }
  }
  
  void createInclineSides(Wall wall, float x, float y, float angle, boolean left, boolean right) {
  ...
  void createIncline(Wall wall, float x, float y, float angle) {
   ...
   void addPointedWindow(int num, float top_margin, float bot_margin, float side_margin, float in_between, float gap, int hue) {
    ...
    if (bot_margin > 0) {
      ...
      createInclineSides(main, main.tl.x, main.tl.y, angle, left_incline, right_incline);
      createInclineSides(base, main.tl.x, main.tl.y, angle, left_incline, right_incline); 
    }
